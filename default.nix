...
in haskellPackages.developPackage {
  ...
  overrides = self: super: {
    cgi = self.callPackage ({ mkDerivation, base, bytestring, containers
      , doctest, exceptions, mtl, multipart, network, network-uri, parsec
      , QuickCheck, time, xhtml, lib }:
      mkDerivation {
        pname = "cgi";
        version = "3001.5.0.0";
    ...
    multipart = self.callPackage
      ({ mkDerivation, base, bytestring, parsec, stringsearch, lib }:
        mkDerivation {
          pname = "multipart";
          version = "0.2.0";
